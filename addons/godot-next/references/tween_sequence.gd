...
func _init(tree: SceneTree) -> void:
	...
	__tree.connect("idle_frame", self, "start")
...
func append_callback(target: Object, method: String, args: Array) -> CallbackTweener:
...
##Starts the sequence manually, unless it's already running
func start() -> void:
...
func pause() -> void:
	assert(__started, "Sequence not started!")
	__tween.stop_all()
	__started = false
...
##Pauses the sequence and sets it to the beginning.
func reset() -> void:
	if __started:
		pause()
	__current_step = 0
...
class PropertyTweener:
	...
	func __start(tween: Tween) -> void:
		if __continue:
			__from = __target.get(__property)
